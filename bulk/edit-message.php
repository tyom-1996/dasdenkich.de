<?php
session_start();
include_once $_SERVER['DOCUMENT_ROOT'] . "/bulk/view-functions/edit-view-functions.php";
include_once $_SERVER['DOCUMENT_ROOT'] . "/includes/language.php";
include_once $_SERVER['DOCUMENT_ROOT'] . "/includes/account-balance.php";


$thisPage = $EditMsgViewFunctions->this_page;
$prices   = $EditMsgViewFunctions->getPrices();
$title    = $lang['title'][$l];
...
$bulk_message   = $EditMsgViewFunctions->bulk_message;
$bulk_image     = $EditMsgViewFunctions->bulk_image;
$bulk_storename = $EditMsgViewFunctions->bulk_storename;

$used_characters = $EditMsgViewFunctions->GetUsedCharacters();
$used_price      = $used_characters > 160 ? $u320_price : $u160_price ;
           ...
           <?php  include_once $_SERVER['DOCUMENT_ROOT'] . "/includes/right-header.php"; ?>
            ...
            <div class="wrap">
               <div class="msg-filter-block" >
                    <h1 class ='msg-filter-title'> Edit Bulk Message </h1>
                    ...
                    <div class="card card-6">
                        <div class="card-body">
                            <form method="POST" name="create_sub_user" enctype="multipart/form-data" action="/bulk/bulk-functions.php">
                                <input type="hidden" name="action" value="edit_bulk_msg">
                                <input type="hidden" name="bulk_id" value="<?=$_GET['id']?>">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <div style="display: flex;justify-content: space-between;align-items: center;/* margin-bottom: 13px; */background: white;border-radius: 10px 10px 0 0;border: 1px solid silver;border-bottom: none;">
                                                ...
                                                <div class="include-str-name-for-msg" style="background: none;margin-bottom: 0;border: none;">
                                                    ...
                                                    <input id="store_name_field" name="store_name_field" type="checkbox" <?php echo strlen($EditMsgViewFunctions->bulk_storename) > 0 ? 'data-check="true" checked' : ''  ?>  value="<?=$_SESSION['user-name']?>">
                                                ...
                                                <div class="characters-count-wrap" style="color: green;padding: 0 12px;"> 
                                                    <input type="text" disabled class="characters-count" value="<?= $used_characters ;?>" style="background: white;padding: 2px;width: 45px;text-align: center;font-weight: 600;">
                                                        characters are used, the price is 
                                                    <input disabled class="characters-price" value="$<?=$used_price?>" style="background: white;padding: 2px;width: 88px;text-align: center;font-weight: 600;" > 
                                                    for a block of 500
                                                </div>
                                            ...
                                            <textarea id="new-message" class="input--style-6"  name="message" maxlength="320" placeholder="Reply to message (limit 320 characters)"><?=$bulk_message?></textarea>
                                            <div class="message-action-block" >
                                                
                                                <?php if(strlen($bulk_image) < 1) :?>
                                                ...
                                                <div class="msg-panel-left">     
                                                    ...
                                                    <input type="hidden" class="image_exist" name="image_exist" value="false" >
                                                ...
                                                <div class="msg-panel-left">     
                                                    <label for="upload-msg" style="display: none; align-items: center; margin-bottom: 0px;">
                                                        <i class="fas fa-image open-msg-image-url"></i>
                                                        <span class="attach-text">Attach a picture</span>
                                                    </label>
                                                    <input type="file" style="display:none;" class="upload-msg-image" name="upload-msg-image" id="upload-msg">
                                                    <input type="hidden" name="image_link" value="<?=$bulk_image?>" >
                                                     <input type="hidden" class="image_exist" name="image_exist" value="true" >
                                                    <div class="upload-preview-block" style="display: block;"> 
                                                        <img src="<?=$bulk_image?>" class="upload-preview" alt="your image"> 
                                                        <span>There is one attachment</span> 
                                                        <i class="fas fa-times remove-upload-image"></i>
                                                    </div>     
                                                </div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        var storename_checkbox;
        
        if(typeof $("#store_name_field").attr('data-check') !== 'undefined'){    
            storename_checkbox = true; 
        } else {
            storename_checkbox = false;
        }
        
        var messages_length = <?=$used_characters?>;
        var old_value       = '';
        var upload_image    = $('.upload-msg-image').val().length > 0 || $('.upload-preview').attr('src').length > 2 ? true : false;
        ...
        $(document).on('input','#new-message',function(e){
            ...
            if(char_count >= 320){
                ...
                $('.characters-count-wrap,.characters-count-wrap input').css({ 'color':"red" })
        ...
        $(document).on('change','#store_name_field',function(e){
            ...
            if(this.checked) {
                ...
                if(upload_image) {
                   ...
                   console.log($('.characters-count').val() ,'-', $(this).val().length)
